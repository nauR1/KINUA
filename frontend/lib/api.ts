export async function api<T = unknown>(
  ...
  if (!response.ok) {
    ...
    if (
      response.status === 401 ||
      (response.status === 403 && body.detail?.code)
    ) {
      window.dispatchEvent(
        new CustomEvent("kinua-access-denied", {
          detail: body.detail?.code ? body.detail : { code: "session_expired" },
        }),
      );
    }

    const message = body.detail?.code
      ? "Acesso indisponível. Consulte o administrador."
