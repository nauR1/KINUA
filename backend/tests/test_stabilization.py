...
@pytest.mark.parametrize(
    "view", ["anterior", "posterior", "lateral_right", "lateral_left"]
)
def test_uploaded_view_is_preserved(auth, view):
    import io
    ...
    a = assessment(auth)
    b = io.BytesIO()
    Image.new("RGB", (100, 200), "white").save(b, "PNG")
    response = auth.post(
        "/assessments/" + a["id"] + "/media",
        files={"file": ("input.png", b.getvalue(), "application/octet-stream")},
        data={"view": view},
    )
...
def test_normalize_supported_formats(fmt):
    import io
    ...
    b = io.BytesIO()
    Image.new("RGB", (100, 200), "white").save(b, fmt)
    content, width, height = normalize_image(b.getvalue())
    assert content.startswith(bytes([255, 216])) and (width, height) == (100, 200)
