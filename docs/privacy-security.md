...
Os landmarks são produzidos no navegador e validados estruturalmente no backend. Isso não autentica a origem visual de um cliente modificado. Treinamento futuro deve usar consentimento/finalidade apropriados, revisão da qualidade, separação de bases e controles adicionais; salvar revisões não autoriza reutilizar dados para treinamento.
